...
(
	cd $BASEDIR/..
	rsync -a -i --exclude=.git --exclude=var/run --delete-after $TOP_DIR/ $BACKEND_USER@$BACKEND_HOST:$TOP_DIR
	rsync -a -i --exclude=.git --exclude=var/run --delete-after $TOP_DIR/ $FRONTEND_USER@$FRONTEND_HOST:$TOP_DIR
)
# kick the service
